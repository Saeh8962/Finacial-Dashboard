...
app.post("/api/toggleAlert",function(req,res){
    var email = req.body.email;
    var symbol= req.body.symbol;
    var movement=req.body.movement;
    var identifier= req.body.identifier;
    var active = req.body.active;
    var status = active ===true? "false":"true"
   


    // console.log(req.body);
    // console.log("s/102",identifier,movement);
    var alert_type = alertType(identifier,movement);
    if(alert_type===0){
        res.json('something went wrong in switch statement')
    }
    var sqltoggleAlert = "UPDATE stock_app.user_alerts SET user_alerts.active =? where email = ? and symbol =? and alert_type =?";

      db.query(sqltoggleAlert,[status,email,symbol,alert_type],function(err,result1){
          console.log("s/111",result1)
          if(err)throw err;
          if(JSON.parse(result1.info).info ===0){
              console.log("no alert column was updated")
              res.json("Disable failed")
              res.end()
          }
          else if(JSON.parsea(result1.info).info ===1){
  ...
  function getUseralerts(email,callback){
    var getAlertQuery= 'select stock_app.user_alerts.symbol,stock_app.Alerts.Identifier,stock_app.Alerts.movement,stock_app.user_alerts.valuechange,stock_app.user_alerts.active from stock_app.user_alerts INNER JOIN stock_app.Alerts on stock_app.user_alerts.alert_type=stock_app.Alerts.id and stock_app.user_alerts.email=?;'
  ...
  app.post('/api/disableAlert',function(req, res){
      ...
      db.query(sqltoggleAlert,[email,alert.symbol,alertId,'flase'],function(err,result1){
          if(err)throw err;
          if(result1.length ===0){
              console.log("no alert column was updated")
              res.json("Disable failed")
