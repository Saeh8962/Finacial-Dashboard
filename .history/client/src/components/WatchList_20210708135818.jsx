...
function ChangeCurPriceColor(props){
  const curValue = props.dataItem["currentPrice" || ""];
...
//     }
        ...
        if(result.type==="trade"){
            user_watchlist.forEach(symbol => {
                ...
                if(tradePrice.length !==0){
                    ...
                    symbol.currentPrice = Math.round(tradePrice[tradePrice.length-1].p); 
                    symbol.CurrentAmountChange= Math.round(symbol.currentPrice-symbol.eodClose);
                    symbol.CurrentPercentChange= Math.round((symbol.CurrentAmountChange/symbol.eodClose)*100)
    ...
    <GridColumn title="Current Price" field="currentPrice" cell={ChangeCurPriceColor}/>
