      ...
      getEodData(symbols){
        if(symbols !== 'No stocks added'){
          ...
          request(fullUrl, { json: true }, (err, res, body) => {
            ...
            var eod=body;
            var data=Object.keys(eod).map((key)=>[eod[key]])
            var cleanedData= Object.entries(data)
            cleanedData=cleanedData[0][1];

            var values = Object.values(cleanedData).map((key)=>[key])
            console.log(Object.entries(cleanedData));
