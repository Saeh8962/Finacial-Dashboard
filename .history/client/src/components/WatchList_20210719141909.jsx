...
function shouldgetUpdatedEod(symbol){
  ...
  var lastTradeDay= currentDate.getDay()+1;
  if(dayOfTheWeek===1){
...
export default withRouter( class WatchList extends React.Component {
        ...
        if(result.type==="trade"){
            user_watchlist.forEach(symbol => {
                ...
                if(tradePrice.length !==0){
                    ...
                    symbol["09. change"]= +(symbol.currentPrice-symbol.eodClose).toFixed(2);
                    symbol.["10. change percent"]= +((symbol["09. change"]/symbol["8. previous close"])*100).toFixed(2);
    ...
    componentDidMount(){
      ...
      if(this.state.stock!=='No stocks added'){
        ...
        var whichToUpdate= this.state.stocks.map(stock=>{
          return shouldgetUpdatedEod(stock['01. symbol'])
        })
        var newList = this.state.stocks.map((stock,indx)=>{
          if(whichToUpdate[indx]){
            return(this.getEodData(stock['01. symbol'],((result)=>{
              stock =result
              return stock
            })));
    ...
    removeStock(htmlEvent,stock){
      if(isMarketOpen()){
       ...
       console.log("Unsubscribing from: ",stock["01. symbol"])
       this.connection.send(JSON.stringify({'type':'unsubscribe', 'symbol': stock["01. symbol"]}))
      }
    ...
    renderStockChart(event){
      console.log(event.dataItem["01. symbol"])
      this.setState({chartStock:event.dataItem["01. symbol"]})
    }
    render(){
