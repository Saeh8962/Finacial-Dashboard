...
function getZillowData(town_r,city_r,state_r,hometype){
    //cleans the input request adress to make it usable
    ...
    //this fetch call retrieves the necessary information to retrieve the full dataset from zillowsAPI
    fetch('https://www.zillow.com/'+address+'/', {
           ...
           //parses the html data from zillow to locate parameter information
            ...
            //the parameter information found is applied to create a url to retrieve either listing for sale or listings for rent
            ...
            else if(hometype=='buy'){ 
                var URL =BuyUrl;
                
            }
            //this fetch call request the listing data from zillows api
            fetch(URL, {
            headers: {
            'Accept': '*/*',
            'Accept-Encoding': 'gzip, deflate, br',
            ...
            'Connection': 'keep-alive',}
                ...
                var ListingInfo = Listings.map(function(house){
                    var listing ={info: house.hdpData}
                    return listing})
            ...
            convertToExcel(address,hometype)
            }))
        }))
}
...
function convertToExcel(address,type){
    ...
    var fileName =type+'_'+address+'_report'+fileid+'.xlsx'
...
getZillowData("","colorado-springs",'ca','rent')
